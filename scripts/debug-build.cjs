// Debug build runner to gather runtime evidence for build hangs (CJS for ESM projects)
...
const log = async (hypothesisId, message, data = {}) => {
  const payload = {
    ...
    runId,
    hypothesisId,
    location: "scripts/debug-build.cjs",
    message,
    data,
    timestamp: Date.now(),
  };
...
const steps = [
  { name: "tsc", cmd: process.platform === "win32" ? ".\\node_modules\\.bin\\tsc" : "node_modules/.bin/tsc", args: ["-b", "--pretty", "false"], hypothesisId: "H1" },
  { name: "vite", cmd: process.platform === "win32" ? ".\\node_modules\\.bin\\vite" : "node_modules/.bin/vite", args: ["build", "--clearScreen", "false"], hypothesisId: "H2" },
];
...
(async () => {
  ...
  for (const step of steps) {
    ...
    const result = spawnSync(step.cmd, step.args, { encoding: "utf8", timeout: 20000 });
