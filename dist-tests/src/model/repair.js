...
exports.repairCurvePoints = exports.repairEnergyCurve = exports.repairSections = exports.repairProject = exports.MIN_SECTION_SEC = exports.supportedSchemaVersion = void 0;
exports.supportedSchemaVersion = 1;
const DEFAULT_TITLE = "Untitled";
const DEFAULT_Y_BANDS = ["Low", "Medium", "High"];
const DEFAULT_TEMPO = { enabled: true, bpm: 120, timeSig: { numerator: 4, denominator: 4 } };
exports.MIN_SECTION_SEC = 0.1; // seconds; prevents zero/negative spans on repair
const CURVE_TIE_EPS = 1e-4;
const isTimebaseView = (value) => value === "time" || value === "measuresComputed" || value === "measuresAbstract";
const isTransitionType = (value) => value === "linear" || value === "step" || value === "curve";
const clamp = (v, min, max) => Math.min(Math.max(v, min), max);
const repairProject = (incoming) => {
    if (incoming.schemaVersion !== exports.supportedSchemaVersion) {
    ...
    const sectionResult = (0, exports.repairSections)(incoming.sections || []);
    warnings.push(...sectionResult.warnings);
    const curveResult = (0, exports.repairEnergyCurve)(incoming.energyCurve);
    warnings.push(...curveResult.warnings);
    const project = {
        ...incoming,
        schemaVersion: exports.supportedSchemaVersion,
...
const normalizeTempoModel = (tempo, warnings) => {
    if (!tempo) {
...
const repairSections = (input) => {
    ...
    const sorted = sanitized.sort((a, b) => a.startSec - b.startSec || a.endSec - b.endSec || a.id.localeCompare(b.id));
    const repaired = [];
    let cursor = 0;
    sorted.forEach((section, idx) => {
        const rawDuration = section.endSec - section.startSec;
        const duration = rawDuration > exports.MIN_SECTION_SEC ? rawDuration : exports.MIN_SECTION_SEC;
...
exports.repairSections = repairSections;
const repairEnergyCurve = (curve) => {
    const localWarnings = [];
    ...
    const pointResult = (0, exports.repairCurvePoints)(curve?.points || [], yMin, yMax);
...
exports.repairEnergyCurve = repairEnergyCurve;
const repairCurvePoints = (input, yMin, yMax) => {
    const warnings = [];
    const sanitized = input
...
exports.repairCurvePoints = repairCurvePoints;
